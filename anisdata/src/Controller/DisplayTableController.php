...
//select records from table
        $query = \Drupal::database()->select('anisdata', 'm');
        $query->fields('m', ['id', 'name', 'mobilenumber', 'email', 'age', 'gender', 'website']);
        $results = $query->execute()->fetchAll();
        $rows = array();
        foreach ($results as $data) {
            $delete = Url::fromUserInput('/anisdata/form/delete/' . $data->id);
            $edit = Url::fromUserInput('/anisdata/form/mydata?num=' . $data->id);
